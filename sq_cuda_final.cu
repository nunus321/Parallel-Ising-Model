...
std::uniform_real_distribution<float> unifReDistr0to1(0, 1.0);
std::normal_distribution<float> normDist(0.0,1.0);

#define PREC 4 //4 for float, 8 for float

#if PREC == 4
    using spin_type = signed char;
#elif PREC == 8
    using spin_type = int;
#else
    using spin_type = int;
#endif
//const float PI=3.141592653287979;
#define tile_w 16
#define tile_h 16
#define ghost 1
...
public:
    ...
    Atoms(vector<spin_type> z, vector<vector<size_t>> neighbours, size_t no_atoms,float J, float K, float D, float B, float kbT)
...
public:
    ...
    float E=0;
    Atoms atoms;
    System(size_t size, float J, float K, float D, float B, float kbT, vector<spin_type> z, vector<vector<size_t>> neighbours)
...
spin_type NewVec(){
    //float p1=unifReDistr0to1(mt)*2-1;
    //float p2=unifReDistr0to1(mt)*2-1;
    spin_type p3=unifIntDistr0to1(mt)*2-1;
...
System MakeLattice(size_t size, float J, float K, float D, float B, float kbT){
    const size_t no_atoms=size*size;

    vector<spin_type>z(no_atoms);
...
__device__ float Ecalc(spin_type* dz, spin_type spin, int i, int L, float J, float B){
    float E=0;
    int no_atoms=L*L;
    ...
    for (int k = 0; k < 4; k++) {
        ...
        //float cross_x = spin[1]*Sjz - spin[2]*Sjy;
        //float cross_y = spin[2]*Sjx - spin[0]*Sjz;
...
float EcalcTot(System& sys, spin_type& spin, size_t& i){
    float E=0;
...
__global__ void SweepAll(spin_type* dz, int L, float J, float B, float kbT,
                           curandState* states, float* dEtot,
                           int parity){
    
    __shared__ spin_type tile[(tile_h +2*ghost)*(tile_w+2*ghost)];
    __shared__ float deltaE;
    ...
    if (t_id < tile_w*tile_h) { 
        ...
        if (should_update && t_id < tile_w*tile_h) {
            spin_type spin = (curand_uniform(&localState) < 0.5f) ? -1 : 1;
            
            spin_type oldspin = tile[int_k];
...
__global__ void Etot(spin_type* dz, int L, float J, float B, float kbT,
                           curandState* states, float* Etot){
...
__global__ void kern_Mag(spin_type* dz, int L, float* M){
    int i=blockIdx.x*blockDim.x+threadIdx.x;
    if(i<L*L) atomicAdd(M,(float)dz[i]/(float)(L*L));
}
...
void MonteCarlo(System& sys, ofstream& filenameE, ofstream& filename, size_t steps,size_t fwrite){
    ...
    spin_type* dz; curandState* dStates; float* dEtot; float* dEi; float* dEf; float* dM;
    
    cudaCheck(cudaMalloc(&dz,       noAtoms*sizeof(spin_type)),"alloc dz");
    cudaCheck(cudaMalloc(&dStates,  noAtoms*sizeof(curandState)),"alloc dStates");
    cudaCheck(cudaMalloc(&dEtot,    sizeof(float)), "alloc dEtot");

    cudaCheck(cudaMemcpy(dz, atoms.z.data(), noAtoms*sizeof(spin_type), cudaMemcpyHostToDevice),"copy dz");
    cudaCheck(cudaMemset(dEtot, 0, sizeof(float)),"set dE");
    cudaCheck(cudaMalloc(&dEi,    sizeof(float)),"dEi alloc");
    cudaCheck(cudaMalloc(&dEf,    sizeof(float)),"dEf alloc");
    cudaCheck(cudaMalloc(&dM,    sizeof(float)),"dM alloc");
    cudaCheck(cudaMemset(dEi, 0, sizeof(float)),"dEi set");
    cudaCheck(cudaMemset(dEf, 0, sizeof(float)),"dEf set");
    cudaCheck(cudaMemset(dM, 0, sizeof(float)),"dM set");
    ...
    cudaCheck(cudaGetLastError(),"rng startup"); 
    cudaCheck(cudaDeviceSynchronize(),"sync start");
    ...
    cudaCheck(cudaMemcpy(&Ei, dEi, sizeof(float), cudaMemcpyDeviceToHost),"copy Ei");
    ...
    for (size_t t = 0; t < steps; t++) {
        ...
        if(t%fwrite==0){
        float currentE=0;
        cudaCheck(cudaMemcpy(&currentE, dEtot, sizeof(float), cudaMemcpyDeviceToHost),"write dE copy");
        sys.E=currentE;
    ...
    cudaCheck(cudaMemcpy(atoms.z.data(), dz, noAtoms*sizeof(spin_type), cudaMemcpyDeviceToHost), "final dz copy");
    ...
    cudaCheck(cudaMemcpy(&Ef, dEf, sizeof(float), cudaMemcpyDeviceToHost),"copy Ef");
    ...
    cudaCheck(cudaMemcpy(&M, dM, sizeof(float), cudaMemcpyDeviceToHost),"copy dM");
